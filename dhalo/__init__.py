...
class DHaloReader(object):
    ...
    def collapsed_mass_history(self, index, nfw_f):  # tree, data, m0, nfw_f):
        ...
        mass_root = self.halo_mass(root)

        for snapshot in np.unique(progenitors["snapshotNumber"]):
            sum_mass = 0
            for halo in progenitors[progenitors["snapshotNumber"] == snapshot]:
                mass = self.halo_mass(halo)
