...
def mock(data_frame=False):
    ...
    Arguments:
        data_frame (bool): whether to return a DataFrame or NumPy array
...
def data(file_hdf5, file_numpy=None, data_frame=False):
    ...
    **Output data format:**
    ...
    particleNumber:
        number of particles in a halo; might differ from masses identified by
        other methods
    hostIndex:
        index of a host halo; for subhaloes, this points to a parent halo; for
        main haloes, this points to themselves
    descendantHost:
        index of a host halo of descendant of a halo (or subhalo); this field
        eliminates "multiple descendance" problem, always creating a merger
        history which works for main progenitors only
    isMainProgenitor:
        1 if it is

    Arguments:
        file_hdf5 (str): filename of an HDF5 data store
        file_numpy (str): filename to which NumPy array object can be saved
            (for faster re-reads);  this is later used for faster data
            retrieval in :func:`src.read.retrieve`;  if ``None``, no data is
            cached
        data_frame (bool): whether to return a DataFrame or not (default not,
            returns NumPy array)
    ...
    d = np.core.records.fromarrays(
        [np.array(f["/haloTrees/%s" % column].value) for column in columns[0]],
        names=columns[0],
        formats=columns[1],
    )
